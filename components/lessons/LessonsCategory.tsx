...
const LessonsCategory: React.FC<LessonsCategoryProps> = ({
  ...
  useEffect(() => {
    const fetchLessonCategory = async () => {
      try {
        ...
        const data = await response.json();
        if (response.ok) {
          console.log('lessonTags', data.data);
          setLessonTags(data.data);
        }
        setIsLoading(false);
      } catch (error) {
  ...
  const cardColors = ['#3e585e', '#1b6c82', '#2e6270', '#3088a0'];

  return (
    <View style={styles.cardsContainer}>
      <View style={styles.cardRowContainer}>
        {lessonTags?.slice(0, 2).map((tag, index) => (
          <LessonCard
            key={tag.id}
            title={tag.title}
      ...
      <View style={styles.cardRowContainer}>
        {lessonTags?.slice(2, 4).map((tag, index) => (
          <LessonCard
            key={tag.id}
            title={tag.title}
            completed={(progressSummary[tag.title] as Record)?.completed || 0}
            ...
            onPress={() => {
              ...
              router.push(`/(tabs)/lessons/level/${tag.title}`);
            }}
